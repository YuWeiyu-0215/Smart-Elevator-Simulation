...
此外，调度内核上还有一项配套机制：**每 30 秒重新评估一次还在等电梯的呼梯请求，必要时改派更合适的电梯**，
对应"满载后调度策略僵化"这个痛点。
...
| 指标 | 含义 |
|---|---|
| 平均候梯时间（按键起算） | 从按下按钮到进入电梯 |
| 平均候梯时间（闸机起算） | 从刷闸机到进入电梯，**两列之差就是找按钮、按按钮花掉的时间** |
| P95 候梯时间 | 95% 的学生候梯时间不超过该值 |
| 最大候梯时间 | 极端情况下最长的一次等待 |
| 平均乘梯时间 | 进入电梯到到达目标楼层 |
| 无效停靠次数 | 空开门的次数，衡量电梯做了多少无用功 |
| 平均每次出发载客人数 | 反映顺路合乘的程度 |
| 电梯运行利用率 | 运行时间占比 |
...
> 中间几行代表"改造只做了一部分"的中间状态；7.4 把每人上下客耗时设为 1 秒、重跑 10 个种子。
...
平峰时人少，两台电梯同时响应，个人确实更快；代价是电梯多空跑、门多空开。
...
默认模型不计乘客上下客耗时（开关门 2.6 s 后直接运行）。
将每人上下客耗时设为 1 秒再跑一遍（10 个种子）：
...
**资料来源：** 电梯相关资料（开关门时间、运行速度、载客量等参数）来源于网上公开资料；调度算法采用 LOOK 电梯调度算法（公开的经典算法思想）。

**代码说明：** 除此之外的全部代码由本人在 AI 工具辅助下编写，未复制任何开源项目的代码。
